  ...
  $.ajax({
    ...
    success: function (response) {
      if (response != "0") {
        ...
        profit = response.incm - response.trns;
        if (profit < 0) {
          $("#total").css("color", "red");
        } else {
...
var KTDatatable_expense = (function () {
      ...
      },
      {
        ...
        template: function (row) {
          return (
            ...
            row.ETID +
            '"    class="edit_tran btn btn-sm btn-clean btn-icon mr-2 " title="Edit details">\
                  ...
                  <a href="javascript:;" id="' +
            row.ETID +
            '"  class="delete_tran btn btn-sm btn-clean btn-icon edit_pr"\
                 title="Delete">\
                      <span class="svg-icon svg-icon-md">\
                          <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">\
                              <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
  ...
  var kt_extension = function () {
    ...
    datatable1.on("datatable-on-check datatable-on-uncheck", function (e) {
  ...
  var edit_expense = function () {
    //edit the transaction from the table
    ...
    $(document).on("click", ".edit_tran", function () {
      ...
      $.ajax({
        //get the transactio attribute and them to the form
      ...
      $("#editextrans").on("submit", function (event) {
        //update transaction form get 3 attributes and update them
  ...
  var selected_export = function () {
    $("#kt_datatable_export_csv").on("click", function () {
      ...
      var excel_name = pr_name + "_" + today;
...
var KTDatatable_income = (function () {
      ...
      },
      {
        ...
        template: function (row) {
          return (
            '<a href="javascript:;" id="' +
            row.ITID +
            '"    class="edit_inc btn btn-sm btn-clean btn-icon mr-2 " title="Edit details">\
                      <span class="svg-icon svg-icon-md">\
                          <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">\
                              <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
                                  <rect x="0" y="0" width="24" height="24"/>\
                  ...
                  <a href="javascript:;" id="' +
            row.ITID +
            '"  class="delete_inc btn btn-sm btn-clean btn-icon edit_pr"\
                 title="Delete">\
                      <span class="svg-icon svg-icon-md">\
                          <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">\
                              <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
  ...
  var edit_income = function () {
    ...
    $(document).on("click", ".edit_inc", function () {
      ...
      $("#updateincmform").on("submit", function (event) {
        //update transaction form get 3 attributes and update them
  ...
  init: function () {
    $("#etransactions").click(function (e) {
      ...
      $("#insrtextrans").on("submit", function (event) {
        $("#expnsname").val($("#dropdownexpns").find(":selected").text()); //add expense name to input inorder to add it to ajax
        console.log($("#dropdownexpns option:selected").text());
        event.preventDefault();
        post_url = $(this).attr("action"); //get form action url
        form_data = $(this).serialize(); //Encode form elements for submission
