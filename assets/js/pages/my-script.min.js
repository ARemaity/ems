    ...
    for (var tryLoader in lds) {
      ...
      if (!loader) {
        errors[errors.length] =
          "- " + tryLoader.toUpperCase() + ": " + this._l(error);
      } else {
        return this._error(error);
      }
    }
    return tryLoader
      ? this._error("no_loader_matched", errors.join("\n"))
  ...
  t.getResponseHeader = function (label) {
    with (this) {
      return _ldObj && _ldObj.getResponseHeader
        ? _ldObj.getResponseHeader(label)
        : null;
    }
  };
  ...
  t._hash2query = function (content, prefix, queryText, queryElem) {
    ...
    if (("" + typeof content).toLowerCase() == "object") {
      ...
      if (
        content &&
        content.parentNode &&
        content.parentNode.appendChild &&
        content.tagName &&
        content.tagName.toUpperCase() == "FORM"
      ) {
...
JsHttpRequest.LOADERS.xml = {
  loader: function (req) {
    JsHttpRequest.extend(req._errors, {
      ...
      xml_no_diffdom:
        "Cannot use XMLHttpRequest to load data from different domain %",
      xml_no_headers:
        "Cannot use XMLHttpRequest loader or ActiveX loader, POST method: headers setting is not supported, needed to work with encodings correctly",
      xml_no_form_upl:
        "Cannot use XMLHttpRequest loader: direct form elements using and uploading are not implemented",
    });
    this.load = function () {
      if (this.queryElem.length) return ["xml_no_form_upl"];
      if (
        this.url.match(new RegExp("^([a-z]+://[^\\/]+)(.*)((:[0-9]*)+)", "i"))
      ) {
        if (
      ...
      xr.onreadystatechange = function () {
        ...
        try {
          ...
          eval(
            "JsHttpRequest._tmp = function(id) { var d = " +
              rtext +
...
JsHttpRequest.LOADERS.form = {
  loader: function (req) {
    JsHttpRequest.extend(req._errors, {
      ...
      form_el_belong_diff:
        'Element "%" belongs to a different form. All elements must belong to the same form!',
      form_el_inv_enctype:
        'Attribute "enctype" of the form must be "%" (for IE), "%" given.',
    });
    this.load = function () {
      var th = this;
      ...
      if (th.queryElem.length) {
        ...
        if (th.method == "POST") {
          ...
          var given =
            (form.attributes.encType && form.attributes.encType.nodeValue) ||
            (form.attributes.enctype && form.attributes.enctype.value) ||
            form.enctype;
          if (given != need) {
